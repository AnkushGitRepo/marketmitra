...
**No build in flight.** Everything below merged to `main`/`v2`, pushed, and **deployed to production 2026-09-13** — work shifts to page-by-page/feature-by-feature refinement, scoped one at a time.
...
- **Index detail pages** (`/dashboard/index/[name]`) — NIFTY 50/SENSEX/NIFTY BANK/INDIA VIX each get a stock-page-like detail page (live quote, 52w hi/lo, price chart via yahoo-finance2, general market news), reached via search or the existing index cards — **deliberately not in the nav**. Verified live in production.
- Mitra navigation + file-based portfolio import ([ADR 0022](./docs/decisions/0022-mitra-navigation-and-file-import.md)) — deployed 2026-09-12; archiving/pruning protocol still deliberately not run.
- The production-readiness pass ([ADR 0023](./docs/decisions/0023-analytics-and-cookie-consent.md)) — legal/security/SEO/a11y/mobile, now pushed and deployed alongside everything above. Privacy Policy/Terms still flagged as needing the user's own lawyer review before being treated as final.

Mobile app development (formerly Phase 12) is dropped — not on the roadmap; that number is now Screener's.
