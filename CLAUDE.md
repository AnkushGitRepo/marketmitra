...
**Mitra navigation + file-based portfolio import** ([ADR 0022](./docs/decisions/0022-mitra-navigation-and-file-import.md)) — page-aware navigation via LLM tool-calling (hard boundary at the tool-definition level: no settings/security/billing/destructive-action tool exists, ever) plus file-based portfolio import (image/XLSX/CSV/DOCX/PDF → extract → match → mandatory preview → explicit confirm, never auto-save). Built (Parts A-D), tested, verified live, merged to `main`/`v2`, and **deployed to production 2026-09-12**. Smoke-tested post-deploy (new routes + chat route return clean 401s, not 500s). Still **not yet run: the archiving/pruning protocol** — hold that for explicit sign-off (deploying isn't the same as "reviewed, collapse the docs"); `docs/architecture.md`'s new section stays the full working reference until then. Mobile app development (formerly Phase 12) is dropped — not on the roadmap.

**Production-readiness pass** (launch checklist: legal pages, security, SEO, accessibility, mobile, reliability — see [ADR 0023](./docs/decisions/0023-analytics-and-cookie-consent.md)) — built, tested, and **deployed to production 2026-09-12**. Privacy Policy/Terms are live but explicitly flagged as needing the user's own lawyer review before being treated as final.

**Live-quote fix (yahoo-finance2 + NSE/BSE) + stock aggregate endpoint** ([ADR 0024](./docs/decisions/0024-live-quote-yahoo-finance2-and-stock-aggregate-endpoint.md)) — dashboard/portfolio prices were stale (the UI never called the real live-quote path at all, deriving price from the latest end-of-day close); fixed by wiring yahoo-finance2 (primary) → NSE → BSE into the dashboard, and removing yfinance from that one path entirely. Also added `GET /api/stock/{ticker}`, a screener.in-style aggregate endpoint. Built and tested on branch `live-quote-fix`, verified live against the real Yahoo Finance API. **Not yet pushed or deployed** — awaiting explicit go-ahead.

Outside the above, work is page-by-page/feature-by-feature refinement, scoped one at a time.
